...
function apply(s: State, action: string): State {
  ...
  switch (action) {
    ...
    case 'rename document to n2':
      // bug: cancels name detection but forgets the summary also reads the name, so a
      // summary build already in flight keeps running with the name it started with
      return s.name === 'n2'
        ? { ...s, name: 'n2', nameSetManually: true }
        : { ...s, name: 'n2', nameSetManually: true, nameDetecting: false };
